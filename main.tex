...
\noindent\enquote{\Large \itshape It doesn't stop being magic just because you found out how it was done.}\bigbreak
...
Research was conducted to investigate numerical models of quantum systems, with a focus on Hydrogenic systems. Several finite difference (FD) methods to obtain instantaneous solutions to the Schr\"{o}dinger equation were compared, and improvements to accuracy (500x more using sparse-diagonal data structures), stability (using central-FD methods), and efficiency (up to 400x speedup using appropriate eigensolver and development of custom `boosting' algorithms) were developed. Two families (FD and Krylov subspace) of time-propagation methods were investigated, extending the capabilities of the models to include time-evolution. The FD propagator outperformed the Krylov propagator in all tests, both efficiency and accuracy where it could be compared, although during low-order propagation there was little difference in efficiency. A parallelisation scheme for these propagators was designed and implemented, and its effiency and scalability measured - finding that it performed well in both serial ($<10\%$ of code serial) and parallel (efficiency/core reducing by under $20\%$ at 32 cores) efficiency testing. Further work was undertaken based on these findings to implement a FD version of RMT; a FD implementation for inner-region propagation was developed and tested, and work towards an outer-region propagator has begun.
\end{abstract}
...
\include{Appendices/AppendixLuke}
